...
export const LiveClassesPage = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-1 border-b border-slate-200 text-xs">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg font-bold transition-colors whitespace-nowrap cursor-pointer select-none text-[11px] sm:text-xs ${
              activeTab === tab.id
      ...
      <div className="admin-card !p-2 sm:!p-3 flex flex-col lg:flex-row items-center justify-between gap-1.5 sm:gap-2.5">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-1.5 sm:gap-2 w-full lg:w-auto">
          ...
          {/* Subject Filter */}
          <div className={`${filterClassHasStreams ? 'col-span-2' : 'col-span-1'} sm:w-44`}>
            <Select
          ...
          <button
            type="button"
            onClick={loadData}
            className="col-span-2 sm:col-span-1 p-1.5 sm:p-2 rounded-lg border border-slate-200 text-slate-500 hover:text-primary-600 hover:bg-slate-50 transition-colors shrink-0 cursor-pointer flex items-center justify-center gap-1 text-xs"
            title="Refresh Live Classes"
          >
            <RefreshCw className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="sm:hidden text-[11px] font-medium">Refresh</span>
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          {/* 1. Mobile High-Density Cards View (< 640px) */}
          <div className="grid grid-cols-1 gap-1.5 sm:hidden">
            {displayedLiveClasses.map((lc) => (
              <div
                key={lc.id}
                className="admin-card !p-2.5 flex flex-col justify-between space-y-2 shadow-2xs hover:border-primary-200 transition-colors"
              >
                {/* Top: Academic Context + Status */}
                <div className="flex items-center justify-between gap-1.5">
                  <div className="flex items-center gap-1 flex-wrap min-w-0">
                    <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-indigo-50 text-primary-700">
                      {lc.className}
                    </span>
                    {lc.streamName && (
                  ...
                  {lc.computedStatus === 'live' ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.2 rounded-full text-[9px] font-bold bg-rose-50 text-rose-700 border border-rose-200 shrink-0">
                      <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping"></span>
                      Live Now
                    </span>
                  ) : lc.computedStatus === 'upcoming' ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.2 rounded-full text-[9px] font-bold bg-blue-50 text-blue-700 border border-blue-200 shrink-0">
                      Upcoming
                    </span>
                  ) : lc.computedStatus === 'completed' ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.2 rounded-full text-[9px] font-bold bg-slate-100 text-slate-600 shrink-0">
                      Completed
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-0.2 rounded-full text-[9px] font-bold bg-amber-50 text-amber-800 border border-amber-200 shrink-0">
                      Cancelled
                    </span>
                  )}
                </div>
                ...
                <div className="pt-1.5 border-t border-slate-100 flex items-center justify-between gap-1.5">
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1 rounded-md border border-slate-200 text-slate-600 hover:text-primary-600 hover:bg-slate-50 transition-colors cursor-pointer"
                  ...
                  <div className="flex items-center gap-0.5">
                    {lc.computedStatus === 'upcoming' && (
                      <button
                        ...
                        className="px-1.5 py-1 rounded-md text-amber-700 hover:bg-amber-50 text-[10px] font-bold cursor-pointer"
      ...
      <Modal
        ...
        maxWidth="max-w-lg"
      >
        <form onSubmit={handleSaveLiveClass} className="space-y-2.5 sm:space-y-3.5">
          <Input
